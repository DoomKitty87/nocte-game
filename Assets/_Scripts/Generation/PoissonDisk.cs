...
public static class PoissonDisk
{
    ...
    public static List<Vector2> GeneratePoints(float radius, Vector2 sampleRegion, int numSamplesBeforeRejection = 30, int seed = 0) {
        ...
        while (spawnPoints.Count > 0) {
            ...
            for (int i = 0; i < numSamplesBeforeRejection; i++) {
                ...
                if (IsValid(candidate, sampleRegion, cellSize, radius, points, grid)) {
    ...
    private static bool IsValid(Vector2 candidate, Vector2 sampleRegion, float cellSize, float radius, List<Vector2> points, int[,] grid) {
        if (candidate.x >= 0 && candidate.x < sampleRegion.x && candidate.y >= 0 && candidate.y < sampleRegion.y) {
