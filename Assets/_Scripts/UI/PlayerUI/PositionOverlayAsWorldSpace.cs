...
public class PositionOverlayAsWorldSpace : MonoBehaviour
{
  ...
  [SerializeField] private Transform _connectionTarget;
  
	private void Start() {
		_overlay.position = Camera.main.WorldToScreenPoint(_target.position) + _offset;
    _overlayConnection.position = Camera.main.WorldToScreenPoint(_connectionTarget.position) + _connectionOffset;
	}
  ...
  public void ResetPosition() {
    ...
    Vector3 connectionPosition = Camera.main.WorldToScreenPoint(_connectionTarget.position) + _connectionOffset;
