...
public class ScanHighlighting : MonoBehaviour
{
  ...
  private List<Transform> _highlightTargets = new List<Transform>();
  ...
  public void Highlight() {
    ...
    for (int i = 0; i < _highlightTargets.Count; i++) {
      ...
      Vector3 endPosition = _highlightTargets[i].position;
      ...
      GameObject highlight = Instantiate(_highlightPrefab, _highlightTargets[i].position, Quaternion.identity);
      highlight.GetComponent<ScanHighlight>()._position = _highlightTargets[i].position;
