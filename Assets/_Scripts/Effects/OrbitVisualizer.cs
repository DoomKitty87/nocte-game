  ...
  public void Initialize() {
    for (int i = 0; i < _moonManager._moons.Length; i++) {
      ...
      for (int j = 0; j < _resolution; j++) {
        ...
        positions[j] = Quaternion.LookRotation(_moonManager._moons[i].orbitAxis) * new Vector3(Mathf.Cos(radians), Mathf.Sin(radians), 0) / transform.parent.localScale.x * _moonManager._moons[i].distance;
      }
