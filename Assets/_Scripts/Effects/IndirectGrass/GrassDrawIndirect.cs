...
public class GrassDrawIndirect : MonoBehaviour
{
    ...
    for (int i = -(_chunkCount - 1) / 2; i < (_chunkCount - 1) / 2 + 1; i++) {
      for (int j = -(_chunkCount - 1) / 2; j < (_chunkCount - 1) / 2 + 1; j++) {
        ...
        RenderChunk chunk = new RenderChunk(
          ...
          _lodValues,
          _positionCompute,
          new Vector2(_cameraPosition.position.x, _cameraPosition.position.z)
        );
  ...
  public void UpdatePlayerPosition(Vector2 playerPosition) {
    Vector2Int moveDelta = new Vector2Int(
      Mathf.FloorToInt(playerPosition.x / _chunkSize) - _middleChunk.x,
      Mathf.FloorToInt(playerPosition.y / _chunkSize) - _middleChunk.y
    );
