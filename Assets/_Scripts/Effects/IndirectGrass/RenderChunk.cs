...
public class RenderChunk
{
  ...
  private void ComputePositions(int lod) {
    ...
    _positionCompute.SetVector(
      "_tileBottomLeftPosition",
      new Vector3(
        Mathf.Floor(_position.x / WorldGenInfo._tileEdgeSize) * WorldGenInfo._tileEdgeSize,
        0,
        Mathf.Floor(_position.z / WorldGenInfo._tileEdgeSize) * WorldGenInfo._tileEdgeSize
      )
    );
    ...
    _positionCompute.Dispatch(
      kernelIndex,
      Mathf.CeilToInt(_chunkDensity[lod] * _chunkDensity[lod] / (float)threadX),
      1,
      1
    );
  }
  ...
  public void Render(Vector2 cameraPosition) {
    ...
    Graphics.DrawMeshInstancedIndirect(
      ...
      0,
      _material,
      new Bounds(new Vector3(0, 0, 0), new Vector3(5000, 5000, 5000)),
      _argsBuffer
    );
  }
