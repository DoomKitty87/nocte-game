...
public class MovementDeprecated : MonoBehaviour
{
	...
	private bool Grounded() {
		Collider[] colliders =
			Physics.OverlapSphere(gameObject.transform.position + _groundCheckOffset, _groundCheckSphereSize);
	...
	private (Vector3, float) SlopeParallelDirAndAngle() {
		...
		Vector3 xzNormal = new(hit.normal.x, 0, hit.normal.z);
	...
	private Vector3 ModifyVectorBySlope(Vector3 input) {
		...
		if (slopeOut.slopeAngle < _maxDownwardsSlopeAngle && !(slopeOut.slopeAngle < 0.1f)) {
			Quaternion slopeRotation =
				Quaternion.AngleAxis(slopeOut.slopeAngle, Vector3.Cross(slopeOut.slopeGradeDirection, Vector3.down));
	...
	private void UpdateInputs() {
		if (_rawJumpInput && !_rawJumpInputLastFrame) _jumpFirstDown = true;
		if (_rawSlideInput && !_rawSlideInputLastFrame) _slideFirstDown = true;
	...
	private void Update() {
		...
		// Ansel badness
		if (_procGen != null)
			_procGen.UpdatePlayerLoadedChunks(transform.position -
	...
	private void FixedUpdate() {
		if (_moveState == MovementState.Walking) {
			...
			if (!Grounded()) {
				...
				_rigidbody.drag = 0;
				if (_inputVector == Vector3.zero) _rigidbody.drag = _moveDrag;
				...
				float speedCap = Mathf.Lerp(_maxMoveSpeedInit, _maxMoveSpeed,
					_timeSinceLastDirectionChange / _moveTimeInitToMax);
				if (_rigidbody.velocity.magnitude >= speedCap) _rigidbody.velocity = _rigidbody.velocity.normalized * speedCap;
			}
		}
		...
		if (_moveState == MovementState.Sliding) {
			...
			if (!Grounded() && !_rawSlideInput) {
				...
				if (_rigidbody.velocity.magnitude < _slideForceExitSpeed) _moveState = MovementState.Walking;
			}
		}
		...
		if (_moveState == MovementState.Falling) {
			...
			if (Grounded() && _groundCheckDelay <= 0) {
				...
				if (hit.collider != null) _accel = Vector3.zero;
	...
	private void OnDrawGizmos() {
		...
		(Vector3 slopeGradeDirection, float slopeAngle) slopeOut = SlopeParallelDirAndAngle();
		Gizmos.DrawLine(transformPosition,
			transformPosition + Vector3.Cross(slopeOut.slopeGradeDirection.normalized, Vector3.up));
